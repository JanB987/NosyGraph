...
`GraphEngine` now exposes copied plain state, composes it through [GraphPhysicsShadowInputService](GraphPhysicsShadowInputService.md), can emit explicit compact observations through [GraphPhysicsShadowObserver](GraphPhysicsShadowObserver.md), and can copy current motion through [LegacyGraphKinematicsAdapter](LegacyGraphKinematicsAdapter.md). The animation loop and active simulation remain unchanged, and there is no default logging. The next implementation increment joins input and frame evidence behind an exact revision check. The manual badge matrix remains an independent activation gate and must still be completed before replacing live legacy mutation.
