...
`GraphEngine` now exposes copied plain state, composes it through [GraphPhysicsShadowInputService](GraphPhysicsShadowInputService.md), can emit explicit compact observations through [GraphPhysicsShadowObserver](GraphPhysicsShadowObserver.md), and joins copied motion through [GraphPhysicsShadowSampleService](GraphPhysicsShadowSampleService.md) behind an exact revision check. The animation loop and active simulation remain unchanged, and there is no default logging. The next implementation increment runs the deterministic replacement engine only through a detached experiment boundary. The manual badge matrix remains an independent activation gate and must still be completed before replacing live legacy mutation.
