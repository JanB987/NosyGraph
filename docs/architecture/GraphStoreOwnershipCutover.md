...
- Delete shadow comparison and the normal-toggle legacy executor only after the [Legacy ownership audit](LegacyOwnershipAudit.md) reports no live or evidence dependents.
- Remove legacy semantic arrays only when no legacy-only feature reads them; the D6 audit currently reports these arrays as live-owned.
...
`GraphEngine` now exposes copied plain state, composes it through [GraphPhysicsShadowInputService](GraphPhysicsShadowInputService.md), can emit explicit compact observations through [GraphPhysicsShadowObserver](GraphPhysicsShadowObserver.md), joins copied motion through [GraphPhysicsShadowSampleService](GraphPhysicsShadowSampleService.md), and exposes only zero-step [GraphPhysicsParityService](GraphPhysicsParityService.md) comparison. The animation loop and active simulation remain unchanged, and there is no default logging. The next implementation phase extracts characterized force calculations into pure units behind the replacement engine. The manual badge matrix remains an independent activation gate and must still be completed before replacing live legacy mutation.
