...
A3 now composes forces -> [GraphMotionIntegrator](GraphMotionIntegrator.md) -> [GraphContainerConfinement](GraphContainerConfinement.md) -> anchoring -> confinement again in [StagedGraphPhysicsEngine](StagedGraphPhysicsEngine.md). [GraphPhysicsAnchoringState](GraphPhysicsAnchoringState.md) supplies the explicit seed and retains updated bounds/history and effective fixed coordinates between steps. A4 reconciles that state across 'setInput' updates by stable container ID, kind, origin, and node ID. Legacy capture, dynamic synchronization, broader host-event reconciliation, and live parity remain pending.
