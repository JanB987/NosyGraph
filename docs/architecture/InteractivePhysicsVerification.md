...
Automated coverage now verifies that active ordinary LinkType relationships render when both endpoint nodes are already visible, without re-enabling global node discovery; badges remain hidden on unselected nodes while selection or show-all mode reveals them; and scaled relationship lines keep a one-screen-pixel minimum. The complete automated suite passes with 88 test files and 403 tests.
...
The first manual retest still showed no relationship lines or badges. Investigation found that Obsidian was loading the installed `.obsidian/plugins/nosygraph` bundle, which had no settings file and therefore used the default `O3/LinkTypes` folder, while this vault stores definitions in `System/LinkTypes`. The live resolver now loads valid definitions explicitly referenced by the graph note regardless of registry-folder location. After that repair, the remaining manual check is that deselected nodes keep their badges hidden while selected nodes reveal them.
