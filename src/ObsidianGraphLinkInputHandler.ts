...
export class ObsidianGraphLinkInputHandler {
  constructor(private readonly app: App) {}

  requestLinkInput(request: GraphBadgeLinkInputRequest): Promise<GraphLinkBadgeDropMutationResult> {
    return new Promise((resolve) => {
      const modal = new GraphLinkInputModal(this.app, request, async (sourceFile) => {
        const source: GraphLinkMutationNodeRef = {
          nodeId: sourceFile.path,
          path: sourceFile.path
        };
        const result = await new ObsidianGraphLinkMutationHandler(this.app).applyBadgeLinkAdd({
...
class GraphLinkInputModal extends Modal {
  private inputEl!: HTMLInputElement;
  private listEl!: HTMLDivElement;
  private query = "";
  private items: LinkInputItem[] = [];
  private selectedIndex = 0;
  private completed = false;

  constructor(
    app: App,
    private readonly request: GraphBadgeLinkInputRequest,
    private readonly onFileSelected: (file: TFile) => Promise<void>,
  ...
  onOpen(): void {
    ...
    this.modalEl.addClass("o3-graph-link-input-modal");

    this.contentEl.createEl("h3", { text: `Link note under ${this.request.property}` });
  ...
  private async chooseItem(item: LinkInputItem): Promise<void> {
    this.completed = true;
    try {
      ...
      console.error("[GraphLinkInput] Failed to add link:", error);
      new Notice("Failed to add graph link.");
    }
  }
  ...
  private buildNewFilePath(title: string): string {
    ...
    const basePath = sanitized.includes("/")
      ? sanitized
      : `${this.getParentFolderPath(this.request.target.path)}${sanitized}`;
  ...
  private getItemTitle(item: LinkInputItem): string {
